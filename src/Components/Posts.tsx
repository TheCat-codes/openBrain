...
import { getPosts, getLikes, getComments } from "../Actions/postsActions"
...
export function Posts () {
  ...
  const { posts, loading, error } = useSelector((state:RootState) => state.postSlice)
  ...
  return (
    ...
    <div className="list-results">
      {posts.length === 0 && !loading && !error && <h2>No posts available</h2>}
            ...
            return (
              <li key={post.id} className="post">
                <div className="creator-data">
                  <img onClick={() => redirectToForeign(post.creator)} src={post.user_image} alt="" title={post.user_image} />
                  <div className="name-created">
                    <strong onClick={() => redirectToForeign(post.creator)}>{post.username}</strong>
